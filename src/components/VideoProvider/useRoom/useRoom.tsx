    ...
    token => {
      ...
      return Video.connect(token, { ...optionsRef.current, tracks: localTracks }).then(
        newRoom => {
          ...
          _printNetworkQualityStats(
            newRoom.localParticipant.networkQualityLevel,
            newRoom.localParticipant.networkQualityStats
          );

          newRoom.localParticipant.on('networkQualityLevelChanged', _printNetworkQualityStats);
